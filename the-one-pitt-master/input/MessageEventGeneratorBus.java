 ...
 * be configured.
 */
public class MessageEventGeneratorBus implements EventQueue {
	...
	/** Range of host addresses that can be receivers only one*/
	 ...
	 */
	public MessageEventGeneratorBus(Settings s){
		...
		if (s.contains(TO_HOST_RANGE_S)) {
			this.toHostRange = s.getInt(TO_HOST_RANGE_S); //cause only 1 dest
		}
	...
	public ExternalEvent nextEvent() {
		...
		MessageCreateEventBus mce = new MessageCreateEventBus(from, to, this.getID(), 
